...
def dataset_preprocess(encoder, f0_predictor, use_diff, vol_aug, skip_loudnorm, num_processes, tiny_enable):
    if precheck_ok:
        ...
        preprocess_commands = [
            ...
            r"python preprocess_flist_config.py --speech_encoder %s %s %s" % (encoder, vol_aug_arg, tiny_arg),
...
def regenerate_config(encoder, vol_aug, tiny_enable):
    ...
    vol_aug_arg = "--vol_aug" if vol_aug else ""
    tiny_arg = "--tiny" if tiny_enable else ""
    cmd = r"python preprocess_flist_config.py --speech_encoder %s %s %s" % (encoder, vol_aug_arg, tiny_arg)
    output = ""
    try:
...
def training(gpu_selection, encoder, tiny_enable):
    ...
    PRETRAIN = { 
        ...
        "cnhubertlarge": ("D_0.pth", "G_0.pth", "pre_trained_model/cnhubertlarge/vol_emb"),
...
def continue_training(gpu_selection, encoder):
    ...
    if len(model_files) == 0:
        return "你还没有已开始的训练"
    cmd = r"set CUDA_VISIBLE_DEVICES=%s && python train.py -c logs/44k/config.json -m 44k" % (gpu_selection)
    subprocess.Popen(["sh", "-c", cmd])
...
def diff_training(encoder, k_step_max):
    ...
    subprocess.Popen(["sh", "-c", r"python train_diff.py -c configs/diffusion.yaml"])
...
with app:
    gr.Markdown(value="""
        ### So-VITS-SVC 4.1-Stable WebUI 推理&训练 v2.3.11
    ...
    with gr.Tabs():
        with gr.TabItem("推理") as inference_tab:
            ...
            with gr.Row():
                ...
                k_step = gr.Slider(label="浅扩散步数，只有使用了扩散模型才有效，步数越大越接近扩散模型的结果", value=20, minimum = 1, maximum = 1000)
            with gr.Row():
        ...
        with gr.TabItem("训练"):
            ...
            with gr.Row():
                ...
                num_processes = gr.Slider(label="预处理使用的CPU线程数，可以大幅加快预处理速度，但线程数过大容易爆显存，建议12G显存设置为2", minimum=1, maximum=multiprocessing.cpu_count(), value=4, step=1)
            with gr.Row():
    ...
    with gr.Tabs():
        ...
        app.queue(concurrency_count=1022, max_size=2044).launch(server_name="0.0.0.0",inbrowser=True,quiet=True,share=True,server_port=9870)